    ...
    findById(_id, callback, toJSON) {
        ...
        if(typeof id !== "object") {
            process.nextTick(function() {
                callback(new Error("you should pass a valid IDs object"));
            });
            return callback.promise;
        }

        const findWithoutCache = function() {
            const promise = self.where(id).findOne(function(err, row) {
                callback(err, row);
            }, toJSON);

            // The outer promise mirrors this result. Handle the internal
            // promise so callback users don't get an extra unhandled rejection.
            promise.catch(emptyFunc);
        };

        if(this.cache) {
            this.cache.getData(this.toshihiko.database, this.model.name, id, function(err, data) {
