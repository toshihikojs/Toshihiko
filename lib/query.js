...
var sqlParser = require("../util/sql_parser");
...
Query.prototype._makeSet = function() {
    ...
    for(var key in this.updateData) {
        ...
        if(typeof v === "string" && v.length >= 4 && 
            ...
            str += "?";
            arr.push(v === null ? null : type.restore(v));
        }
    }
 ...
 * @private
 */
Query.prototype._makeForceIndex = function() {
    if(!this._forceInex) {
        return "";
    }

    return "FORCE INDEX(\`" + this._forceInex + "\`) ";
};
 ...
 * @returns {Query}
 */
Query.prototype.index = function (forceIndex) {
    this._forceInex = forceIndex;
    return this;
};
...
Query.prototype.field = function(fields) {
    ...
    this.fields = λ.compact(fields.split(","));
    this.fields = this.fields.map(function(name) {
        return name.trim();
    });
    return this;
};
    ...
    if(!this.model.cache) {
        ...
        this.toshihiko.execute(sql, function(err, rows) {
            ...
            self.model.cache.getData(self.toshihiko.database, self.model.name, rows, function(err, data) {
                ...
                var dataFinder = function(row, n) {
                    return self._cacheFitWithPrimary(row, n);
                };
                for(var i = 0; i < rows.length; i++) {
                    var d = λ.find(data, dataFinder.bind(null, rows[i]));
