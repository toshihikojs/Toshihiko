...
Query.prototype._makeSet = function() {
    ...
    for(var key in this.updateData) {
        if(!this.updateData.hasOwnProperty(key)) continue;
    ...
    if(!this.model.memcached) {
        ...
        this.toshihiko.execute(sql, function(err, rows) {
            if(err) {
                ...
                if(!findOne) promisor._onExecuteResult(err, undefined, sql);
                return;
            }
            ...
            for(var i = 0; i < rows.length; i++) {
                ...
                if(withJson) rows[i] = rows[i].toJSON();
            }

            // callback and set promisor
            callback(undefined, rows, sql);
            if(!findOne) promisor._onExecuteResult(undefined, rows, sql);
        });
    } else {
        ...
        this.toshihiko.execute(sql, function(err, rows) {
            if(err) {
                ...
                if(!findOne) promisor._onExecuteResult(err, undefined, self.makeSQL("find"));
                return;
            }
            ...
            self.model.memcached.getData(self.toshihiko.database, self.model.name, rows, function(err, data) {
                ...
                var taskDone = function(TO) {
                    ...
                    if(!findOne) promisor._onExecuteResult(err, result, newSql);
                };
