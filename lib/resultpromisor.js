...
ResultPromisor.prototype._copyResults = function() {
    var res = [];
    for(var i = 0; i < this._result.length; i++) res.push(this._result[i]);
    return res;
};
...
ResultPromisor.prototype.success = function(callback) {
    if(!this._resultGot) this.onSuccess.push(callback);
    else if(!this._err) this._runSuccess(callback);
...
ResultPromisor.prototype.error = function(callback) {
    if(!this._resultGot) this.onError.push(callback);
    else if(this._err) this._runError(callback);
...
ResultPromisor.prototype.finished = function(callback) {
    if(!this._resultGot) this.onCallback.push(callback);
    else this._runCallback(callback);
...
ResultPromisor.prototype._onExecuteResult = function(err, result) {
    ...
    if(this._result.length >= 1) this._result.shift();

    // error | success
    if(this._err) {
        while(this.onError.length) {
            var cb = this.onError.shift();
            this._runError(cb);
        }
    } else {
        while(this.onSuccess.length) {
            var cb = this.onSuccess.shift();
            this._runSuccess(cb);
        }
    }

    while(this.onCallback.length) {
        var cb = this.onCallback.shift();
