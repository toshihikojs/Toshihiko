...
class Yukari {
    ...
    validateOne(name, value, callback) {
        ...
        async.eachSeries(field.validators, function(validator, next) {
            ...
            return (validator.bind(self.$model))(value, next);
        }, callback);
    }
     ...
     */
    delete(conn, callback) {
        if(typeof conn === "function") callback = conn, conn = null;
        ...
        const self = this;
        this.$model.where(pk).conn(conn).limit("0, 1").delete(function(err, result, sql) {
    ...
    update(conn, callback) {
        ...
        async.waterfall([
            self.validateAll.bind(self),
            function(callback) {
                self.$adapter.update(self.$model, conn, pk, data, callback);
            }
        ], function(err, extra) {
            if(err) return callback(err);
