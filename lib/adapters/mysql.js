...
class MySQLAdapter extends Adapter {
    ...
    makeFind(model, options) {
        ...
        sql += options.count ?
            ...
            fields ?
                fields.map(field => {
                    const column = model.nameToColumn[field];
                    if(!column) {
                        throw new Error(`no field named "${field}" in model "${model.name}"`);
                    }
                    return `\`${column}\``;
                }).join(", ") :
                "*";
    ...
    findWithNoCache(model, callback, options) {
        ...
        this.execute(options.conn, sql, function(err, rows) {
            if(err) {
    ...
    findWithCache(cache, model, callback, options) {
        ...
        this.execute(options.conn, pkSQL, function(err, rows) {
            if(err) {
            ...
            cache.getData(self.database, model.name, rows, function(err, data) {
                ...
                function fetchFromMySQL(taskObject) {
                    ...
                    self.execute(options.conn, sql, function(err, row) {
                        if(err) {
                            errors.push(err);
                            return taskObject.done();
                        }
    ...
    count(query, callback) {
        ...
        this.execute(query._conn, sql, function(err, rows) {
    ...
    updateByQuery(query, callback) {
        ...
        async.waterfall([
            function(callback) {
                ...
                self.execute(query._conn, relatedSql, function(err, result) {
            ...
            function(callback) {
                ...
                self.execute(query._conn, sql, callback);
            }
        ], function(err, result) {
    ...
    insert(model, conn, data, callback) {
        ...
        this.execute(conn, sql, function(err, row) {
            if(err) return callback(err, undefined, sql);
            if(!row) {
            ...
            _model.where(where).findOne(function(err, row) {
                if(err) {
                    return callback(err, undefined, sql);
                }
    ...
    update(model, conn, pk, data, callback) {
        ...
        async.waterfall([
            function(callback) {
                ...
                self.execute(conn, relatedSql, function(err, result) {
                    if(err) return callback(err);
                    model.cache.deleteKeys(self.getDBName(), model.name, result, function(err) {
                        return callback(err);
                    });
            ...
            function(callback) {
                self.execute(conn, sql, function(err, results) {
                    if(err) return callback(err);
     ...
     * @param {Function} callback the callback function
     */
    execute(conn, sql, params, callback) {
